...
class FilterManager:
    def __init__(self):
        self.default_scenarios = {
            'reduce_a': 10.0,
            'reduce_peak': 15.0,
            'reduce_top_entity': 20.0,      # новое название
            'investments': 50000.0
        }
    ...
    def render_sidebar(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if 'date' in df.columns:
            ...
            date_range = st.sidebar.date_input("📅 Диапазон дат", value=(min_d, max_d), min_value=min_d, max_value=max_d)
            if len(date_range) == 2:
                filter_state['date_range'] = (date_range[0], date_range[1])
            else:
                filter_state['date_range'] = (min_d, max_d)

        st.sidebar.divider()
        st.sidebar.header("🔮 What-if сценарии")

        filter_state['reduce_a'] = st.sidebar.slider("Снижение A-класса (%)", 0.0, 100.0, self.default_scenarios['reduce_a'], step=1.0)
        filter_state['reduce_peak'] = st.sidebar.slider("Снижение в пиковые дни (%)", 0.0, 100.0, self.default_scenarios['reduce_peak'], step=1.0)
        filter_state['reduce_top_entity'] = st.sidebar.slider("Снижение в топ-объектах (80%) (%)", 0.0, 100.0, self.default_scenarios['reduce_top_entity'], step=1.0)
        filter_state['investments'] = st.sidebar.number_input("Инвестиции (₽)", 0.0, value=self.default_scenarios['investments'], step=10000.0)
